...
void arg_set(char *serialdevice, int baudrate, char *name)
{
    ...
    strncpy(arguments.serialdevice, serialdevice, MAX_DEV_STR_LEN);
    strncpy(arguments.name, name, MAX_DEV_STR_LEN);
}
...
{
    snd_seq_event_t* ev;
    char bytes[] = {0x00, 0x00, (char)0xFF};
...
void* read_midi_from_serial_port(void* seq)
{
    ...
    while (run)
    {
        ...
        /* parse MIDI message */
        else parse_midi_command((snd_seq_t*)seq, port_out_id, buf);
    }
    pthread_exit (0);
}
...
//	ioctl(serial, TIOCSSERIAL, &ser_info);
   ...
   {
     fprintf (stderr, "pthread_create error for thread Read_midi_from_alsa\n");
     exit (1);
  }
    ...
    if (pthread_create(&midi_in_thread, NULL, read_midi_from_serial_port, (void*) seq)< 0)
    {
      fprintf (stderr, "pthread_create error for thread Read_midi_from_serial_port\n");
      exit (1);
    }
    return 1;
}
...
void ttymidi_off(void)
{
    ...
    (void)pthread_join(midi_out_thread, &status);
    (void)pthread_join(midi_in_thread, &status);
